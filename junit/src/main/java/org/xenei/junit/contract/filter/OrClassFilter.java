...
/**
 * A ClassFilter providing conditional OR logic across a list of
 * class filters. This filter returns {@code true} if any filters in the
 * list return {@code true}. Otherwise, it returns {@code false}.
 * Checking of the class filter list stops when the first filter returns
 * {@code true}.
 *
 */
public class OrClassFilter extends AbstractConditionalClassFilter implements Serializable {
     ...
     */
    public OrClassFilter(final List<ClassFilter> classFilters) {
        super(classFilters);
    }
