...
public class ContractMojo extends AbstractMojo {
	...
	private void doReportInterfaces(final InterfaceReport ir) {
		...
		try {
			...
			bw.newLine();
			bw.newLine();
			for (final Class<?> cls : ir.getClassFilter().filter(
		...
		if (!untestedInterfaces.isEmpty()) {
			if (untested.isReporting()) {
				...
				try {
					...
					bw.newLine();
					bw.newLine();
					for (final Class<?> c : untested.getFilter().filter(
							untestedInterfaces)) {
						bw.write(c.getName());
						bw.newLine();
					}
			...
			if (untested.isFailOnError()
					&& !untested.getFilter().filter(untestedInterfaces)
							.isEmpty()) {
		...
		if (!unimplementedTests.isEmpty()) {
			if (unimplemented.isReporting()) {
				...
				try {
					...
					bw.newLine();
					bw.newLine();
					for (final Class<?> c : unimplemented.getFilter().filter(
							unimplementedTests)) {
						bw.write(c.getName());
						bw.newLine();
					}
			...
			if (unimplemented.isFailOnError()
					&& !unimplemented.getFilter().filter(unimplementedTests)
							.isEmpty()) {
